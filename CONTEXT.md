...
**Gap turn**:
One **AI Assistant** turn that asks one genuine gap on a create or recovery **Assistant task** (create target, Location, **Feedback**, new-Offer terms, Offer title, Campaign Draft title, audience, or channel), with no persist that turn. The ask uses operator words. It does not use schema field names such as value, validity, type, or required usage. Facts the operator already named stay on the create; a later Gap does not ask for them again. A new Offer title is drafted by the server; this turn does not ask for the Offer name. If the next send does not fill the open gap: a confused send gets an explanation of the last ask; a clear **Retrieve** or create is answered; the gap stays resumable unless the operator cancels. When saved **Analysis scope** is **All owned locations**, unnamed create asks Location; **Recovery path** **Feedback** gaps may include venue in row labels. Distinct from **Clarify**, from a **Compare turn**, from a completing **live answer**, and from a refusal.
_Avoid_: Clarify (when meaning this), Draft interview, Allowed clarification, Ask-back (as this noun), Value (as the spoken ask), Validity (as the spoken ask), Reply with one exact label (as the required reply)
