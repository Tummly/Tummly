...
**Feedback**:
One guest submission captured via the Private feedback form for an Owned location after a **QR link** scan. Owns the guest-provided fields (name, contact, comment, **Offers opt-out**), submission time, required `QrCodeId` (source **QR code** / **QR type** via join), **Feedback workflow status**, and — as they are introduced — **AI classification** (sentiment and **Detected Tags**), operator corrections, **Feedback internal notes**, **Feedback close-out** facts, and per-submission activity history. References a **Location Guest** and does not own it. Latest activity and the future Feedback page are entry points onto Feedback; they do not own those details. Activity history records things that happened on that Feedback (e.g. received; later classified, corrected, note added, note deleted, **Feedback workflow status** changed, **Feedback close-out**) — not pending pipeline hints. Note body edits and same-to-same status no-ops do not create activity history rows. For the Feedback details slice, activity history is derived from Feedback facts (submission time, **Feedback internal notes** including soft-deletes, operator **classification correction** rows, workflow status change facts, and **Feedback close-out** facts) rather than a separate Feedback activity store. Correction, note, status-change, and close-out events carry the operator display name, timestamp, and action (including from/to sentiment for corrections, from/to status for workflow changes, and intent plus **Feedback close-out reason** for close-outs). A workflow status change that belongs to a close-out is shown as the close-out beat, not a second bare status-change beat.
...
The canonical operator follow-up lifecycle of one **Feedback**. Three persisted values: **New**, **In progress**, **Resolved**. Initial value on guest submit is always **New**, independent of **AI classification** outcome or sentiment. Operators may move among **New** and **In progress** (including skip); same-to-same is a no-op. **Resolved** is set only by **Feedback close-out** or successful **Feedback recovery** completion — not by a bare status set. Reopen moves **Resolved** → **In progress**. Changes only via explicit operator action — opening **Feedback details** or selecting an inbox row does not auto-advance status. Starting **Feedback recovery** advances **New** → **In progress** (leaves **In progress** unchanged); abandoning the wizard mid-flow does not revert status. Each real change produces an activity-history beat (actor, timestamp, from → to); close-out shows as a close-out beat rather than a bare status-change beat. Inbox tabs: **All** (no status filter); **Needs attention** (derived); **New** / **In progress** / **Resolved** (exact status match). Distinct from **AI classification** status, from derived **Needs attention**, from the 24-hour freshness badge **New** (Feedback), and from the inbox **All** tab (unfiltered view, not a status).
...
_Avoid_: Needs recovery (when meaning Feedback), attention status, priority status, workflow Needs attention, Home Needs attention (when meaning this Feedback signal alone)
...
_Avoid_: Dismiss feedback, close ticket, no-action status (as a workflow status)

**Feedback close-out reason**:
The operator-selected reason recorded on a **Feedback close-out** fact. Shared reason set for **Mark resolved** and **Mark no action needed**. Persisted with close-out intent so Resolved-via-close-out stays distinguishable in activity/history from other paths to **Resolved**, and so the two close-out CTAs stay distinguishable when the reason is the same. Other requires a mandatory **Feedback internal note**; other reasons do not.
_Avoid_: Resolution reason (alone), dismiss reason, no-action reason (as separate enums)
...
**Gap turn**:
One **AI Assistant** turn that asks one genuine gap on a create or recovery **Assistant task** (create target, Location, **Feedback**, new-Offer terms, Offer title, Campaign Draft title, audience, or channel), with no persist that turn. The body is a **Gap ask**. Facts the operator already named stay on the create; a later Gap does not ask for them again. A new Offer title is drafted by the server; this turn does not ask for the Offer name. A unique natural reply fills a listed choice; an exact catalogue label is not required. If several hard rules are missing, the next ask is the first of: type, value, required usage, when the offer should end, then placement only if they asked to attach. If the next send does not fill the open gap: a confused send gets an explanation of the last **Gap ask**; a clear **Retrieve** or **Refuse** is answered and the gap stays resumable; a new create drops the open gap, tells the operator the previous draft ask was dropped, and runs the new create; cancel drops the gap with no new create. When saved **Analysis scope** is **All owned locations**, unnamed create asks Location; **Recovery path** **Feedback** gaps may include venue in row labels. Distinct from **Clarify**, from a **Compare turn**, from a completing **live answer**, and from a refusal.
_Avoid_: Clarify (when meaning this), Draft interview, Allowed clarification, Ask-back (as this noun), Reply with one exact label (as the required reply)

**Gap ask**:
The operator-facing wording of a **Gap turn**. Product-owned. Not the model live-answer body. Uses operator words. Does not use schema field names such as value, validity, type, or required usage. When the open gap is when the offer should end, the ask allows a date or how many days after a guest gets the offer.
_Avoid_: Value (as the spoken ask), Validity (as the spoken ask), model body (as this wording), Reply with one exact label
...
**Guest tag**:
One operator-facing label on a **Location Guest**, drawn from that restaurant’s **Guest tag catalog**. Same concept whether applied via Guests Add Tag or via union when **AI classification** Succeeds with **Detected Tags** (including operator **Edit tags** replace/promote on Feedback). Distinct from a **Detected Tag**, which lives only on Feedback under **AI classification**. On Succeeded classification (and backfill of existing Succeeded Feedback, and operator Detected Tag edits that leave Succeeded), each Detected Tag is also applied as a Guest tag on that Feedback’s Location Guest — additive union only. Sentiment-only classification correction does not change Guest tags; removing a Guest tag is only via explicit Guests tagging flows; removing Detected Tags from Feedback does not remove Guest tags.
...
_Avoid_: Recovery queue, negative guests (as the metric name); Positive feedback (the Smart Group — different membership rule); Home Needs attention (Guests is not a first-cut source)
...
The Home-scoped module for the Operator dashboard Home body. Depends on the Operator workspace session’s selected Owned location. Owns Home location-scoped loads (feedback snapshot; **Performance overview** KPI counts for the **Home performance date range**; Finish-setting-up acknowledgements via an internal ack module; **Home Needs attention** assembly from source queues; **Weekly brief** load, lazy generate, and retry; **Home Recommended next step** load, retry, session dismiss, and soft cache), Latest activity Feedback details via an internal Feedback details module, a narrowed Home body view-model (selected venue, Smart Guest Link caps, setup steps, KPIs, activity — not shell chrome), Preview guest form (Smart Guest Link, then acknowledge on the ack module), and Copy Smart Guest Link (clipboard copy of the selected location’s Smart Guest Link; toast on success). Does not own Download QR. Swappable later for other page modules without tearing down the shell or workspace session.
_Avoid_: Operator Home session (when meaning shared shell state), Home controller as the owner of locations/profile

**Operator Reports page module**:
The Reports-scoped module for the Operator dashboard Reports hub, child reports, and **Reports Weekly Brief** page. Depends on the Operator workspace session’s selected Owned location. Owns **Reports Weekly Brief** load, lazy generate, and presentation for the hub “This week's guest loop”, Generate brief controls, and `/reports/weekly-brief` (shared **Weekly brief** body via Home Weekly brief APIs). Does not own the Weekly brief durable store or Monday job. KPI read ownership for hub and child reports is locked with the Reports query model; this module is the Reports page-module home for those surfaces when wired.
_Avoid_: Operator Reports session; Home Weekly brief controller as Reports UI owner
...
**Weekly brief**:
The live Operator Home AI summary for the selected Owned location covering the closed prior workspace week in that location’s timezone. Workspace week bounds and generate day come from **Week starts on** (for example Friday–Thursday when the start day is Friday). Grain: one durable row per location + workspace week; immutable after success. Same body feeds **Reports Weekly Brief** surfaces. Generate on the next configured start day in location TZ (hourly job) with lazy generate on Home or Reports if the row is missing. Changing **Week starts on** does not rewrite succeeded rows; Home, Reports, and the job use the new closed week at once. On success: Notification type `weekly-brief-ready` under **Weekly brief reminders**; CTA opens the **Reports Weekly Brief** page for that location. Free call. Home section owned by the **Operator Home page module**. Wire: `/api/home/weekly-brief`. Distinct from **Reports Weekly Brief** (Reports surfaces), **Home Recommended next step**, **Campaign recommendation**, and **Offer recommendation**. Post-ship one-time all-locations generate is ops only, not ongoing product behaviour.
_Avoid_: Home Recommended next step; Campaign recommendation; Offer recommendation; Reports Weekly Brief (when meaning only the Reports hub/page); empty shell only; ISO week (when **Week starts on** is not Monday)

**Reports Weekly Brief**:
The Reports surfaces that show the shared **Weekly brief** body for the selected Owned location: hub “This week's guest loop”, Generate brief controls, and `/reports/weekly-brief`. Same closed-prior-workspace-week grain and generate pattern as Home (**Week starts on**, job + lazy). Does not replace Home **Weekly brief**. Independent of Reports KPI date-range controls. Free call. Owned by the **Operator Reports page module** for those surfaces; uses Home Weekly brief APIs (no parallel store). Hub ready state: headline plus short secondary copy. Page v1: same structured body as Home. Mock dialog, PDF download, and Mark as reviewed are out of the product lock. Distinct from Home **Weekly brief** and from date-ranged Reports KPI reads.
_Avoid_: Weekly brief (when meaning only the Home section); Reports brief; insight brief; AI Weekly Brief Last 7 days
...
The shared operator drawer that shows one **Feedback** for an Owned location the operator controls. UI title is **Feedback details**. Entry points: Home Latest activity, Guests Guest details (stacked), Guest Profile / Edit, and the Feedback page inbox. Loads details from the backend (not from the list row as source of truth). Header venue is `{Location name} · {QR source}` when source is known (else Location name); Submission details use Location name under the Figma **Restaurant** label and **Address** under **Location** — not `Restaurant.Name`. Shows derived **New** (freshness) and **Needs attention** badges; absolute submitted time; live **Feedback workflow status** control; **Issue tags** (UI title for **Detected Tags**, editable via **Edit tags** when classification is Succeeded or Failed); Contact state / Contact availability from contact facts; **Feedback reference** `FDB-{padded id}`; QR source from the Feedback’s `QrCodeId` join. Previous/Next list navigation is Feedback-page inbox only. **AI classification** lifecycle drives Pending / Succeeded / Failed empty states. **View guest profile** when a Location Guest link exists and the operator is not already on that guest’s Profile, Edit, or Guest details; correct classification when Succeeded; **Edit tags** when Succeeded or Failed (Pending stays disabled); **Feedback internal notes** create / edit / soft-delete. Activity history includes received, note added/deleted, classification corrected, Detected Tags updated, and workflow-status changes. Recovery / campaign / Message guest CTAs stay pending chrome where not yet shipped.
...
**Detected Tag**:
One value from the product-fixed closed vocabulary of topic themes on a Feedback (e.g. Food quality, Wait time). Assigned as a multi-label set by **AI classification**, independent of sentiment — positive, neutral, and negative Feedback may all carry tags. Operators may also replace the set via **Edit tags** on **Feedback details** when classification is Succeeded or Failed (Failed promote requires sentiment in the same dialog and becomes Succeeded). On **Feedback details** the UI section title is **Issue tags**. Inbox / filters may still say Issue tags for the same vocabulary. `Other` is exclusive and never combined with another tag. Distinct from a **Guest tag** on a Location Guest; on classification Succeeded (including operator Edit tags promote/replace) a Detected Tag may also be applied as a Guest tag (additive union only — removals do not remove Guest tags). Sentiment-only classification correction does not change Detected Tags or Guest tags.
...
**Offers catalog**:
Reusable offer definitions operators create and manage on the Operator **Offers** page and attach to Campaigns, **Feedback recovery**, and Guest form thank-you (benefit type + values, purchase rules, title/description, validity, staff instructions, status). Benefit **type** is fixed after create; later definition edits (values, purchase rules, validity, copy) apply to **new Offer issues** only — existing passes keep what they had. Distinct from historical Feedback-scoped one-off **Recovery offer** rows (pre-cutover) and from Guest Loop setup offer strings. Attach model: `.scratch/recovery-catalog-offers/PRD.md`; build: `.scratch/recovery-catalog-offers/IMPLEMENT.md`. Shared Create/Edit Offer drawer; Edit field lock: `.scratch/offers/issues/15-edit-offer-field-parity.md`. Browse/select of preexisting catalog offers unlocks with the Offers list.
