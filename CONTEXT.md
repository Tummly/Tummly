...
**Gap turn**:
One **AI Assistant** turn that asks one genuine gap on a create or recovery **Assistant task** (create target, Location, **Feedback**, new-Offer terms, Offer title, Campaign Draft title, audience, or channel), with no persist that turn. The body is a **Gap ask**. Facts the operator already named stay on the create; a later Gap does not ask for them again. A new Offer title is drafted by the server; this turn does not ask for the Offer name. A unique natural reply fills a listed choice; an exact catalogue label is not required. If several hard rules are missing, the next ask is the first of: type, value, required usage, when the offer should end, then placement only if they asked to attach. If the next send does not fill the open gap: a confused send gets an explanation of the last **Gap ask**; a clear **Retrieve** or create is answered; the gap stays resumable unless the operator cancels. When saved **Analysis scope** is **All owned locations**, unnamed create asks Location; **Recovery path** **Feedback** gaps may include venue in row labels. Distinct from **Clarify**, from a **Compare turn**, from a completing **live answer**, and from a refusal.
