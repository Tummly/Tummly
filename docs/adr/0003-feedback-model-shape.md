...
The form captures three required fields: guest name, guest contact (a single string holding either an email or a phone number, guest's choice), and a comment. A `ContactType` column (`Email` | `Phone` | `Unknown`) records a heuristic classification of the contact string at submission time, so future operator-reply features know which channel to use without re-parsing.
