...
import {
  applyPendingFeedbacksFilters,
  beginFeedbacksDatePick,
  clearAllFeedbacksPending,
  clearFeedbacksDate,
  DATE_PRESET_LABELS,
  FEEDBACKS_DETECTED_TAG_OPTIONS,
  isFeedbacksFiltersApplyDirty,
  pickFeedbacksDatePreset,
  projectFeedbacksFilterChips,
  removePendingFeedbacksFilterChip,
  SENTIMENT_LABELS,
  toggleFeedbacksDetectedTag,
  toggleFeedbacksSentiment,
...
type FeedbacksFiltersDialogProps = {
  open: boolean
  session: FeedbacksFiltersPanelSession | null
  onSessionChange: (session: FeedbacksFiltersPanelSession) => void
  onOpenChange: (open: boolean) => void
  onApply: (filters: GuestFeedbacksFilterSelection) => void
}
...
function RemovableChips({
  ...
  chips: FeedbacksFilterChip[]
  onRemove: (chip: FeedbacksFilterChip) => void
}) {
  if (chips.length === 0) {
    return null
  }
...
export function FeedbacksFiltersDialog({
  open,
  session,
  onSessionChange,
  onOpenChange,
  onApply,
}: FeedbacksFiltersDialogProps) {
  ...
  const dirty = isFeedbacksFiltersApplyDirty(session)
  const pendingChips = projectFeedbacksFilterChips(session.pending)
  const sentimentChips = pendingChips.filter((c) => c.kind === "sentiment")
  const tagChips = pendingChips.filter((c) => c.kind === "detected-tag")
  const dateChips = pendingChips.filter((c) => c.kind === "date")

  const removeChip = (chip: FeedbacksFilterChip) => {
    onSessionChange(removePendingFeedbacksFilterChip(session, chip))
  }
      ...
      >
                    ...
                    {(
                      ...
                      <li key={id}>
                        <Button
                          ...
                          variant="ghost"
                          role="option"
                          aria-selected={session.pending.sentiment.includes(id)}
                          className={cn(
                            SELECT_ITEM_CLASS,
                          ...
                          onClick={() => {
                            onSessionChange(
                              toggleFeedbacksSentiment(session, id)
                            )
                          }}
                        >
                          {label}
              ...
              <Popover open={tagsOpen} onOpenChange={setTagsOpen}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={cn(
                      SELECT_TRIGGER_CLASS,
                      session.pending.detectedTags.length === 0 &&
                        "text-[#7d7d7d]"
                    )}
                  >
                    <span>
                      {session.pending.detectedTags.length > 0
                        ? `${session.pending.detectedTags.length} selected`
                        : "Select"}
                    </span>
                    <ChevronDownIcon className="size-4 shrink-0 opacity-60" />
                  </Button>
                ...
                <PopoverContent align="start" className={SELECT_MENU_CLASS}>
                  <ul className={SELECT_LIST_CLASS} role="listbox">
                    {FEEDBACKS_DETECTED_TAG_OPTIONS.map(([id, label]) => (
                      <li key={id}>
                        <Button
                          type="button"
                          variant="ghost"
                          role="option"
                          aria-selected={session.pending.detectedTags.includes(
                            id
                          )}
                          className={cn(
                            SELECT_ITEM_CLASS,
                            session.pending.detectedTags.includes(id) &&
                              SELECT_ITEM_ACTIVE_CLASS
                          )}
                          onClick={() => {
                            onSessionChange(
              ...
              <Popover
                ...
                onOpenChange={(next) => {
                  ...
                  if (next) {
                    setDateCustomStep(false)
                    setDraftRange(undefined)
                    onSessionChange(beginFeedbacksDatePick(session))
                  } else {
                    setDateCustomStep(false)
                    setDraftRange(undefined)
                    onSessionChange({
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              onSessionChange(
                                pickFeedbacksDatePreset(session, "custom", {
                    ...
                    <ul className={SELECT_LIST_CLASS} role="listbox">
                      <li>
                        <Button
                          ...
                          onClick={() => {
                            onSessionChange(
                              pickFeedbacksDatePreset(session, "any-time")
                            )
                            setDateOpen(false)
                          }}
                        >
                      ...
                      {(
                        ...
                        <li key={id}>
                          <Button
                            ...
                            onClick={() => {
                              onSessionChange(
                                pickFeedbacksDatePreset(session, id)
                              )
                              setDateOpen(false)
                            }}
                          >
                      ...
                      {session.pending.date.kind !== "none" ? (
                        <li>
                          <Button
                            ...
                            onClick={() => {
                              onSessionChange(clearFeedbacksDate(session))
                              setDateOpen(false)
                            }}
                          >
                            Clear date
        ...
        <DialogFooter className="gap-3 sm:justify-between">
          <Button
            ...
            variant="ghost"
            onClick={() => {
              onSessionChange(clearAllFeedbacksPending(session))
            }}
          >
            Clear all
          </Button>
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              disabled={!dirty}
              onClick={() => {
                onApply(applyPendingFeedbacksFilters(session))
                resetPopovers()
              }}
            >
              Apply filters
