...
describe("resolveCampaignsListEmptyStateKind", () => {
...
describe("campaignsListSearchMissLabel", () => {
  it("returns Figma search-miss copy for a non-empty query", () => {
    expect(campaignsListSearchMissLabel("weekend")).toBe(
      "No campaigns found for “weekend”"
    )
  })

  it("returns null when search is blank", () => {
    expect(campaignsListSearchMissLabel("  ")).toBeNull()
  })
})
