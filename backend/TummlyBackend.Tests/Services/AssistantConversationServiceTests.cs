...
{
    public partial class AssistantConversationServiceTests : IDisposable
    {
        ...
        public async Task SendTurn_EverywhereCreateWhenAllSaved_IsLocationGap()
        {
            ...
            var answer = ok.Conversation.Messages[^1];
            Assert.Equal("gap", answer.Class);
            Assert.Contains("venue", answer.Body, StringComparison.OrdinalIgnoreCase);
        ...
        [Fact]
        public async Task SendTurn_OfferPath_ModelExtractedOneRuleGap_StoresShrunkStateAndModelQuestion()
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            var ask = "Create a 25% off lunch offer";
            var question = "When should diners be able to use their discount?";
            _fake.SucceedWith(
                AssistantMessageClass.Clarify,
                null,
                question,
            ...
            var answer = ok.Conversation.Messages[^1];
            Assert.Equal("gap", answer.Class);
            Assert.Equal(AssistantGapAsk.EndDateAsk, answer.Body);
            Assert.Empty(answer.Actions);
            Assert.Equal(0, await _context.CatalogOffers.CountAsync());
            ...
            Assert.Equal(ask, gapState.SourceUserMessage);
            Assert.Equal(["validity"], gapState.OpenRules);
        ...
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            var ask = "Create a 20% off lunch offer";
            var monthEnd = EndOfCurrentMonth();
            ...
            // Turn 1 also runs the history-free title generation call.
            _fake.EnqueueSucceedWith(
                AssistantMessageClass.Grounded,
                "Lunch offer",
            ...
            var answered = Assert.IsType<AssistantTurnOutcome.Ok>(
                await _service.SendTurnAsync(
                    ...
                    FirstSendRequest(
                        locationId,
                        "It should expire at the end of this month",
                        started.Conversation.Id
                    )
                )
            );
            ...
            Assert.Equal("grounded", answer.Class);
            Assert.False(answered.Conversation.DraftInterviewActive);
            Assert.Equal(
                "review-offer",
                Assert.Single(answer.Actions).Type
            );
            var offer = Assert.Single(_context.CatalogOffers);
            Assert.Equal(CatalogOfferStatus.Draft, offer.Status);
            Assert.Equal(CatalogOfferType.PercentageDiscount, offer.OfferType);
        ...
        [Fact]
        public async Task SendTurn_OfferAsk_ProviderWithoutTerms_GapUsesProductAsk()
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            _fake.SucceedWith(
                AssistantMessageClass.Grounded,
        ...
        public async Task SendTurn_OfferPathMissingValidity_TermsGapThenPersists()
        {
            ...
            // The model re-extracts every term from the whole thread on the
            // resume turn; the fake mirrors that contract.
            _fake.SucceedWith(
                AssistantMessageClass.Grounded,
                "Lunch discount",
                "Saving the lunch discount.",
        ...
        [Fact]
        public async Task SendTurn_InstructionLikeReplacementValue_GapZeroCatalogRows()
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
        ...
        public async Task SendTurn_OfferPathGenericAttach_IsPlacementTermsGapZeroCatalogRows()
        {
            ...
            Assert.Equal(AssistantTask.OfferPath, gapState.AssistantTask);
            Assert.NotNull(gapState.OfferTermsJson);
            Assert.Contains("placement", gapState.OpenRules);
        }
        ...
        public async Task SendTurn_OfferPathConflictingBenefits_ThenNamesOne_PersistsDraft()
        {
            ...
            _fake.SucceedWith(
                AssistantMessageClass.Grounded,
                "Dessert discount",
                "Saving the percentage discount.",
                AssistantTask.OfferPath,
                null,
                new AssistantOfferPathTermsState
                {
        ...
        [Fact]
        public async Task SendTurn_RetrieveDuringLocationGap_KeepsGapAndPersistsNothing()
        {
            var soho = await SeedLocationAsync(ownerUserId: 7, "Soho");
            await SeedSecondLocationAsync(ownerUserId: 7, "Camden");
            ...
            Assert.Equal(0, await _context.Campaigns.CountAsync());
            Assert.NotEqual("gap", retrieved.Conversation.Messages[^1].Class);
            Assert.DoesNotContain(
                retrieved.Conversation.Messages[^1].Actions,
                action => action.Type == "review-campaign"
            );
            var kept = await StoredGapStateAsync(started.Conversation.Id);
            Assert.Equal(AssistantGapTurn.KindLocation, kept.Kind);
        }
        ...
        [Fact]
        public async Task SendTurn_OfferPathTermsGap_FourteenDays_StaysOnOfferAndPersists()
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            var started = Assert.IsType<AssistantTurnOutcome.Ok>(
                await _service.SendTurnAsync(
            ...
            var answered = Assert.IsType<AssistantTurnOutcome.Ok>(
                await _service.SendTurnAsync(
                    ownerUserId: 7,
                    FirstSendRequest(locationId, "14 days", started.Conversation.Id)
                )
            );
            Assert.Null(answered.Conversation.PendingRecoveryDraft);
            Assert.DoesNotContain(
                answered.Conversation.Messages[^1].Actions,
                action => action.Type == "open-recovery"
            );
            var offer = Assert.Single(_context.CatalogOffers);
            Assert.Equal(CatalogOfferStatus.Draft, offer.Status);
        ...
        [Fact]
        public async Task SendTurn_OfferPathLocationGap_ShortFill_StaysOnOfferAndPersists()
        {
            var camden = await SeedLocationAsync(ownerUserId: 7, "Camden");
            await SeedSecondLocationAsync(ownerUserId: 7, "Soho");
        ...
        [Fact]
        public async Task SendTurn_OfferPathTermsGap_VoucherFill_DoesNotStartRecovery()
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            var started = Assert.IsType<AssistantTurnOutcome.Ok>(
                await _service.SendTurnAsync(
            ...
            _fake.SucceedWith(
                AssistantMessageClass.Clarify,
                null,
                "How long should the 25% discount stay valid?",
                AssistantTask.OfferPath,
                null,
                new AssistantOfferPathTermsState
                {
            ...
            var filled = Assert.IsType<AssistantTurnOutcome.Ok>(
                await _service.SendTurnAsync(
                    ownerUserId: 7,
                    FirstSendRequest(locationId, "voucher", started.Conversation.Id)
                )
            );
            Assert.Equal("gap", filled.Conversation.Messages[^1].Class);
            Assert.Null(filled.Conversation.PendingRecoveryDraft);
            Assert.Empty(filled.Conversation.Messages[^1].Actions);
            Assert.Equal(0, await _context.CatalogOffers.CountAsync());
            await AssertOfferPathGap(started.Conversation.Id, AssistantGapTurn.KindOfferTerms);
        }

        [Theory]
        [InlineData("respond to the guest")]
        [InlineData("help me recover")]
        [InlineData("Draft a recovery response")]
        public async Task SendTurn_DraftInterview_TargetSwitch_DropsOfferGapAndRunsRecovery(
            string recoveryAsk
        )
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            ...
            await AssertOfferPathGap(offer.Conversation.Id, AssistantGapTurn.KindOfferTerms);

            var switched = Assert.IsType<AssistantTurnOutcome.Ok>(
                await _service.SendTurnAsync(
                    ownerUserId: 7,
                    FirstSendRequest(locationId, recoveryAsk, offer.Conversation.Id)
                )
            );
            Assert.Contains(
            ...
            Assert.NotEqual(
                AssistantGapTurn.KindOfferTerms,
                AssistantGapTurn.Parse(
                    (await _context.AssistantConversations.SingleAsync(
                        row => row.Id == offer.Conversation.Id
                    )).DraftInterviewJson
                )?.Kind
            );
        }
        ...
        [Fact]
        public async Task SendTurn_OfferPathTermsGap_DetectMultiTarget_DropsOfferGap()
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            await SeedFeedbackAsync(locationId, DateTime.UtcNow.AddHours(-1));
            var started = Assert.IsType<AssistantTurnOutcome.Ok>(
            ...
            Assert.Contains("Offer", replaced.Conversation.Messages[^1].Body, StringComparison.Ordinal);
            Assert.Contains(
                "Feedback recovery",
                replaced.Conversation.Messages[^1].Body,
                StringComparison.Ordinal
        ...
        [Fact]
        public async Task SendTurn_OfferPathTermsGap_CancelThenRecovery_StartsRecovery()
        {
            var locationId = await SeedLocationAsync(ownerUserId: 7, "Camden");
            await SeedFeedbackAsync(locationId, DateTime.UtcNow.AddHours(-1));
            var offer = Assert.IsType<AssistantTurnOutcome.Ok>(
            ...
            Assert.Equal(
                "open-recovery",
                Assert.Single(recovery.Conversation.Messages[^1].Actions).Type
            );
            Assert.NotNull(recovery.Conversation.PendingRecoveryDraft);
            Assert.Equal(0, await _context.CatalogOffers.CountAsync());
        }
        ...
        private async Task AssertOfferPathGap(int conversationId, string kind)
        {
            ...
            var gap = AssistantGapTurn.Parse(row.DraftInterviewJson);
            Assert.NotNull(gap);
            Assert.Equal(kind, gap.Kind);
            Assert.Equal(AssistantTask.OfferPath, gap.AssistantTask);
        }

        private static DateOnly EndOfCurrentMonth()
        {
            var now = DateTime.UtcNow;
            return new DateOnly(now.Year, now.Month, DateTime.DaysInMonth(now.Year, now.Month));
        }
