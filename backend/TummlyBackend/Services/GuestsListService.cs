...
namespace TummlyBackend.Services
{
    public class GuestsListService : IGuestsListService
    {
        ...
        private async Task<Dictionary<int, GuestFeedbackStats>> LoadFeedbackStatsAsync(
            int locationId
        )
        {
            var projections = await _context.Feedbacks
                .AsNoTracking()
                .Where(f =>
                    f.RestaurantLocationId == locationId
                    && f.LocationGuestId != null
                )
                .Select(f => new FeedbackProjection
                {
            ...
            return projections
                .GroupBy(projection => projection.LocationGuestId)
                .ToDictionary(
                    group => group.Key,
                    group => BuildFeedbackStats(group.ToList())
                );
        }

        private static GuestFeedbackStats BuildFeedbackStats(
            List<FeedbackProjection> feedbacks
        )
        {
        ...
        private static DerivedGuestRow DeriveRow(
            ...
            IReadOnlyDictionary<int, GuestFeedbackStats> feedbackStats,
            DateTime utcNow,
        ...
        )
        {
            ...
            return new DerivedGuestRow
            {
                ...
                MarketingStatus = DeriveMarketingStatus(
                    locationGuest,
                    masterGuest
                ),
        ...
        private static string DeriveMarketingStatus(
            LocationGuest locationGuest,
            MasterGuest masterGuest
        )
        {
            if (locationGuest.OffersOptOut)
            {
                return "Not eligible";
            }

            if (!string.IsNullOrWhiteSpace(masterGuest.Email))
            {
                return "Eligible — Email";
            }

            if (!string.IsNullOrWhiteSpace(masterGuest.Mobile))
            {
                return "Eligible — SMS";
            }
        ...
        private sealed class FeedbackProjection
        {
            ...
            public ClassificationStatus ClassificationStatus { get; init; }

            public FeedbackSentiment? Sentiment { get; init; }
        }

        private sealed class GuestFeedbackStats
        {
